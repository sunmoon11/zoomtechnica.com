window.MP.Loader.add('parent_editor', function () {
    try {
        (function ($) {
            can.Construct('MP.Editor', 
            {
                ...
                prepareEventName: function (name) {
        ...
        (function ($) {
            can.Control('CE.Iframe', 
            {
                ...
                mpceTextarea: null,
                wpEditorId: 'content',
                mpceEditorId: 'motopresscecontent'
            }, 
            {
                minWidth: null,
                gridObj: null,
                ...
                init: function (el) {
                    ...
                    CE.Iframe.wpTextarea = $('#wp-content-editor-container #' + CE.Iframe.wpEditorId);
                    CE.Iframe.mpceTextarea = $('textarea#' + CE.Iframe.mpceEditorId);
                ...
                getTinyMCEContent: function () {
                    if (wp.hasOwnProperty('autosave')) {
                        ...
                        var editor = typeof tinymce !== 'undefined' && tinymce.get(CE.Iframe.mpceEditorId);
                        if (editor && !editor.isHidden())
                            editor.save();
                        var postData = { post_title: $('#title').val() || '' };
                    }
                    ...
                    if (typeof tinyMCE !== 'undefined') {
                        var wpActiveMode = MP.Utils.getEditorActiveMode($('#wp-' + CE.Iframe.wpEditorId + '-wrap').prop('class').split(' '));
                        var mpceActiveMode = MP.Utils.getEditorActiveMode($('#wp-' + CE.Iframe.mpceEditorId + '-wrap').prop('class').split(' '));
                        var hasSwitchto = switchEditors.hasOwnProperty('switchto');
                        CE.Iframe.wpEditor = tinyMCE.get(CE.Iframe.wpEditorId);
                        CE.Iframe.mpceEditor = tinyMCE.get(CE.Iframe.mpceEditorId);
                        if (wpActiveMode === 'html-active') {
                            if (hasSwitchto) {
                                switchEditors.switchto($('#' + CE.Iframe.wpEditorId + '-tmce')[0]);
                            } else {
                                switchEditors.go(CE.Iframe.wpEditorId, 'tmce');
                            }
                            if (typeof CE.Iframe.wpEditor === 'undefined' || CE.Iframe.wpEditor === null) {
                                $.when(motopressCE.tinyMCEInited[CE.Iframe.wpEditorId]).done(function (editor) {
                                    CE.Iframe.wpEditor = editor;
                                });
                            }
                        }
                        if (mpceActiveMode === 'html-active') {
                            if (hasSwitchto) {
                                switchEditors.switchto($('#' + CE.Iframe.mpceEditorId + '-tmce')[0]);
                            } else {
                                switchEditors.go(CE.Iframe.mpceEditorId, 'tmce');
                            }
                            if (typeof CE.Iframe.mpceEditor === 'undefined' || CE.Iframe.mpceEditor === null) {
                                $.when(motopressCE.tinyMCEInited[CE.Iframe.mpceEditorId]).done(function (editor) {
                                    CE.Iframe.mpceEditor = editor;
                                    self.renderContent(title, self.getEditableContent(CE.Iframe.mpceEditor), pageTemplate);
                                });
                            } else {
                                self.renderContent(title, self.getEditableContent(CE.Iframe.mpceEditor), pageTemplate);
                            }
                ...
                getEditableContent: function (editor) {
                    if (editor) {
                        ...
                        return CE.Iframe.mpceTextarea.val().length ? switchEditors._wp_Autop(CE.Iframe.mpceTextarea.val()).replace(new RegExp('\n', 'g'), '') : '';
                    }
                },
        ...
        (function ($) {
            ...
            { myThis: null }, 
            {
                ...
                '#motopress-content-editor-close click': function () {
                    ...
                    CE.Iframe.mpceEditor.focus();
                    if (CE.Iframe.mpceEditor.hasOwnProperty('iframeElement')) {
                        CE.Iframe.mpceEditor.iframeElement.blur();
                    }
                    this.iframe.remove();
        ...
        (function ($) {
            var wpRevision = MP.WpRevision;
            CE.Save = can.Construct({
                contentSavedInCE: false,
                compareString: '',
                mpceEditor: null,
                init: function () {
                    this.mpceEditor = tinyMCE.get('motopresscecontent');
                },
                saveWithReload: function () {
                    this.triggerBeforeSave();
                    this.makeContentNonDirty();
                ...
                triggerBeforeSave: function () {
                    MP.Editor.triggerEverywhere('BeforeSave');
                },
                makeContentNonDirty: function () {
                    wpRevision.setBuilderDirty(false);
                    wpRevision.makeWpNonDirty();
                },
                convertAndInsert: function () {
                    ...
                    if (dom.length) {
                        ...
                        if (parent.CE.Iframe.mpceEditor !== null) {
                            parent.CE.Iframe.mpceEditor.setContent(src, { format: 'html' });
                        }
                        parent.CE.Iframe.mpceTextarea.val(parent.switchEditors._wp_Nop(src));
                    }
                },
                ...
                changeContent: function () {
                    wpRevision.setBuilderDirty(true);
                },
