...
*/
	...
	public static function rowUniversal($atts, $content, $tag) {
...
//			wp_enqueue_style('mpce-theme');
        ...
        return self::rowUniversal($atts, $content, $tag);
	}
    ...
    function mp_span($atts, $content = null, $tag) {
        ...
        return '<div class="mp-span' . $col . ' motopress-clmn ' . self::handleCustomStyles($mp_custom_style, $tag) . $classes . self::getMarginClasses($margin) . self::getBasicClasses($tag, true) . $mp_style_classes . '"' . $style . '>' . do_shortcode($content) . '</div>';
    }
    ...
    function mp_heading($atts, $content = null, $tag) {
        ...
        return '<div class="motopress-text-obj' . self::handleCustomStyles($mp_custom_style, $tag) . $classes . self::getMarginClasses($margin) . self::getBasicClasses($tag, true) . $mp_style_classes . '">' . $result . '</div>';
    }
...
//                        wp_enqueue_script('mp-lightbox');
    ...
    function mp_image_slider($atts, $content = null, $tag) {
        ...
        if (!empty($mp_style_classes)) $mp_style_classes = ' ' . $mp_style_classes;
        $sliderHtml .= '<div class="motopress-image-slider-obj motopress-flexslider mp-flexslider-obj flexslider' . $classes . self::getMarginClasses($margin) . self::handleCustomStyles($mp_custom_style, $tag, true) . '" id="' . $uniqid . '">';
        if (empty($error)) {
            if (!empty($images)) {
...
//                        'terms' => $custom_tax_terms,
        ...
        $postSliderHtml = '<div class="motopress-posts_slider-obj mp-flexslider-obj flexslider' . self::handleCustomStyles($mp_custom_style, $scTag) . self::getMarginClasses($margin) . self::getBasicClasses($scTag, true) . $mp_style_classes .'" data-post-type="'. $post_type .'" id="' . $uniqeid . '">';
        $slide_counter = 0;
        $link = '';
        $error = '';
        $i = 0;
...
//											wp_enqueue_script('mp-lightbox');
        ...
        $galleryHtml = '<div class="motopress-grid-gallery-obj' . self::handleCustomStyles($mp_custom_style, $tag) . self::getBasicClasses($tag, true) . $mp_style_classes . self::getMarginClasses($margin) . $needRecalcClass . $oneColumnClass . '" id="' . $uniqid . '">';
        if (empty($error)) {
...
//                wp_enqueue_script('mp-grid-gallery');
    ...
    function mp_code($atts, $content = null, $tag) {
        ...
        return '<div class="motopress-code-obj' . self::handleCustomStyles($mp_custom_style, $tag) . $classes . self::getMarginClasses($margin) . self::getBasicClasses($tag, true) . $mp_style_classes . '">' . do_shortcode($content) . '</div>';
    }
    ...
    function mp_space($atts, $content = null, $tag) {
        ...
        return '<div class="motopress-space-obj' . self::handleCustomStyles($mp_custom_style, $tag) . self::getBasicClasses($tag, true) . $mp_style_classes  . self::getMarginClasses($margin) . '"><div></div></div>';
    }
...
//			wp_enqueue_script('mp-waypoint-animations');
    ...
    function mp_countdown_timer($attrs, $content = null, $tag){
        ...
        $result = '<div class="motopress-countdown_timer' . self::handleCustomStyles($mp_custom_style, $tag) . $classes . self::getMarginClasses($margin) . self::getBasicClasses($tag, true) . $mp_style_classes . '">';
    ...
    function mp_embed($attrs, $content = null, $tag) {
        ...
        if (!empty($mp_style_classes)) $mp_style_classes = ' ' . $mp_style_classes;
        $result .= '<div class="motopress-embed-obj' . self::handleCustomStyles($mp_custom_style, $tag) . (($fill_space == 'true' || $fill_space == '1') ?
            " fill-space" : "") . $classes . self::getMarginClasses($margin) . self::getBasicClasses($tag, true) . $mp_style_classes . '">' . $embed . '</div>';
    ...
    function mp_quote($attrs, $content = null, $tag) {
        ...
        if (!empty($mp_style_classes)) $mp_style_classes = ' ' . $mp_style_classes;
        if ($cite && $cite_url) {
            $result = '<div class="motopress-quotes' . self::handleCustomStyles($mp_custom_style, $tag) . $classes . self::getMarginClasses($margin) . self::getBasicClasses($tag, true) . $mp_style_classes . '"><blockquote><p>'. $quote_content .'</p></blockquote><p style="text-align:right;"><a href="'.$cite_url.'">'.$cite.'</a></p></div>';
        } elseif ($cite) {
            $result = '<div class="motopress-quotes' . self::handleCustomStyles($mp_custom_style, $tag) . $classes . self::getMarginClasses($margin) . self::getBasicClasses($tag, true) . $mp_style_classes . '"><blockquote><p>'. $quote_content .'</p></blockquote><p style="text-align:right;">'.$cite.'</p></div>';
        } else {
            $result = '<div class="motopress-quotes' . self::handleCustomStyles($mp_custom_style, $tag) . $classes . self::getMarginClasses($margin) . self::getBasicClasses($tag, true) . $mp_style_classes . '"><blockquote><p>'. $quote_content .'</p></blockquote></div>';
        }
	...
	function mp_members_content($attrs, $content = null, $tag) {
		...
		if (!is_user_logged_in()) {
			...
			$result = '<div class="motopress-members-content' . self::handleCustomStyles($mp_custom_style, $tag) . $classes . self::getMarginClasses($margin) . self::getBasicClasses($tag, true) . $mp_style_classes . '">' . str_replace('%login%', $text, $message) . '</div>';
		} else {
			$content = trim($content);
			$members_content = $content ? $content : $members_content;
			$result = "<div class='motopress-members-content" . self::handleCustomStyles($mp_custom_style, $tag) . $classes . self::getMarginClasses($margin) . self::getBasicClasses($tag, true) . $mp_style_classes . "'>" . do_shortcode($members_content) . "</div>";
		}
...
//        wp_enqueue_script('mp-social-share');
	    self::enqFrontScript();
//        $result = '<div class="motopress-share-buttons ' . $align . ' ' . $size . ' ' . $style . $classes . self::getMarginClasses($margin) . $custom_class . self::getBasicClasses($tag, true) . $mp_style_classes . '">';
    ...
    function mp_social_profile($attrs, $content = null, $tag) {
        ...
        if (!empty($mp_style_classes)) $mp_style_classes = ' ' . $mp_style_classes;
//        $socialProfileHtml = '<div class="motopress-social-profile-obj motopress-text-align-' . $align . ' motopress-buttons-' . $size . 'x' . $size . ' motopress-buttons-' . $style . self::getMarginClasses($margin) . $classes .  $custom_class . self::getBasicClasses($tag, true) . $mp_style_classes . '">';
...
//        wp_enqueue_script('mp-google-charts');
        ...
        $result = "<div id=\"". $id ."\" class=\"motopress-google-chart" . self::handleCustomStyles($mp_custom_style, $tag) . $classes . self::getMarginClasses($margin) . self::getBasicClasses($tag, true) . $mp_style_classes .  "\" data-chart=\"". $content ."\"></div>";
    ...
    function mp_wp_audio($attrs, $content = null, $tag) {
        ...
        if (!empty($mp_style_classes)) $mp_style_classes = ' ' . $mp_style_classes;
        $result = do_shortcode( '<div class="motopress-audio-object' . self::handleCustomStyles($mp_custom_style, $tag) . $classes . self::getMarginClasses($margin) . self::getBasicClasses($tag, true) . $mp_style_classes .  '" id="' . $blockID .'"' . $audioTitle .'>'. $shortcode . '</div>');
    ...
    function mp_tabs($attrs, $content = null, $tag) {
        ...
        $tabsHtml = '<div class="motopress-tabs-obj' . self::handleCustomStyles($mp_custom_style, $tag) . $classes . ' motopress-tabs-padding-' . $padding . self::getMarginClasses($margin) . self::getBasicClasses($tag, true) . $mp_style_classes . '" id="tabs' . $uniqid . '">';
        ...
        if ($tabsMatched) {
            ...
            if (!empty($navHtml)) {
                ...
                if (!self::isContentEditor() && $rotate !== 'disable') {
    ...
    function mp_tab($attrs, $content = null, $tag) {
        ...
        $tabsHtml = '<div class="motopress-tab' . self::getMarginClasses($margin) . self::handleCustomStyles($mp_custom_style, $tag) . self::getBasicClasses($tag, true) . '" id="' . $id . '">' . do_shortcode($content) . '</div>';
    ...
    function mp_accordion($attrs, $content = null, $tag) {
        ...
        if (!empty($mp_style_classes)) $mp_style_classes = ' ' . $mp_style_classes;
        $uniqid = uniqid();
//        $accordionHtml = '<div class="motopress-accordion-obj' . $classes . ' motopress-accordion-'. $style . self::getMarginClasses($margin) . $custom_class . self::getBasicClasses($tag, true) . $mp_style_classes . '" id="' . $uniqid . '">';
        $accordionHtml = '<div class="motopress-accordion-obj' . $classes . self::handleCustomStyles($mp_custom_style, $tag) . self::getMarginClasses($margin) . self::getBasicClasses($tag, true) . $mp_style_classes . '" id="' . $uniqid . '">';
    ...
    function mp_service_box($attrs, $content = null, $tag){
        ...
        switch($icon_effect) {
            case 'grayscale' :
                global $motopressCESettings, $is_IE;
	            $scriptSuffix = $motopressCESettings['script_suffix'];

                if ($is_IE) {
                    wp_enqueue_script('mp-service-box-ie-fix', $motopressCESettings['plugin_dir_url'] . 'includes/js/mp-service-box-ie-fix' . $scriptSuffix . '.js');
                }
...
//				wp_enqueue_script('mp-lightbox');
    ...
    function mp_cta($atts, $shortcodeContent = null, $tag) {
        ...
        // "animation" field
		if ( !self::isContentEditor() && ( $animation !== 'none' || $button_animation !== 'none' || $icon_animation !== 'none' ) ) {
...
//			wp_enqueue_script('mp-waypoint-animations');
	...
	public static function isContentEditor() {
	    if (is_null(self::$isContentEditor)) {
		    self::$isContentEditor = self::hasBuilderParam() || self::isBuilderAjax();
	    }
    ...
    public static function hasBuilderParam() {
		return isset($_GET['motopress-ce']) && $_GET['motopress-ce'] === '1';
    }

    public static function isBuilderAjax() {
		// TODO: Also `motopress_ce_render_shortcodes_string`, `motopress_ce_render_template`
		return (
			isset($_POST['action']) &&
