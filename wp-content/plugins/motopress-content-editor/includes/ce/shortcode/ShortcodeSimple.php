...
if (!class_exists('MPCEShortcodeSimple')) {
	...
	class MPCEShortcodeSimple extends MPCEShortcodeBase {
		...
		function mp_quote($atts, $content, $tag) {
			...
			$res = "<blockquote><p>{$quote_content}</p></blockquote>";
			if ($cite && $cite_url) {
	            $res .= '<p style="text-align:right;"><a href="'.$cite_url.'">'.$cite.'</a></p>';
	        } elseif ($cite) {
	            $res .= '<p style="text-align:right;">'.$cite.'</p>';
	        }
		...
		function mp_wp_audio($atts, $content, $tag) {
			...
			if ($mediaIsSet) {
				$src = "src=\"{$audioURL}\"";
				$autoplay = ($autoplay == 'true' || $autoplay == 1) ? ' autoplay="on"' : null;
				$loop = ($loop == 'true' || $loop == 1) ? ' loop="on"' : null;

				$res = "[audio {$src}{$autoplay}{$loop}]";
			}
