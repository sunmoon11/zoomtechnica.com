...
//				wp_redirect(get_edit_post_link($postID, false));
		...
		public function addFieldsToEditPostForm() {
			...
			if (self::isEditorAvailableForPost($post)) {
				...
				$wpTitle = $motopressCELang->CEWPTextEditor;
				$mpceTitle = $motopressCELang->CE;
				...
				<?php if ($mpceEnabled) : ?>
					...
					<p class="description"><?php echo $motopressCELang->CEVisualBuilderTabDescription ?></p>
				<?php endif; ?>
		...
		function wpPostRevisionFields($fields) {
			...
			$fields[self::CONTENT_EDITOR_ID] = $motopressCELang->CE;
